...
dftbuffer, on the same lines as the dftbuffer in sms-tools). Then compute the M point DFT of the 
...
def testRealEven(x):
    ...
    for i in range(1, M / 2 + 1):
        delta = X[i] - X[M - i]
        if abs(delta) > zeroish:
            isRealEven = False

    return (isRealEven, dftbuffer, X)

tc1 = loadTestCases.load(3, 1)
tc2 = loadTestCases.load(3, 2)
isRealEven, dftbuffer, X = testRealEven(tc2['input']['x'])
