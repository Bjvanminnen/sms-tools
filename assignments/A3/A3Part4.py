...
def suppressFreqDFTmodel(x, fs, N):
    ...
    band = fs / N
    band70 = int(np.ceil(70 / band)) + 1

    mX, pX = dftAnal(x, w, N)

    y = dftSynth(mX, pX, w.size) * outputScaleFactor

    mXfilt = np.ones(len(mX)) * -120
    mXfilt[band70:] = mX[band70:]

    yfilt = dftSynth(mXfilt, pX, w.size) * outputScaleFactor
    
    return (y, yfilt)

# tc1 = loadTestCases.load(4, 1)
# tc2 = loadTestCases.load(4, 2)
# y, yfilt = suppressFreqDFTmodel(**tc1['input'])
